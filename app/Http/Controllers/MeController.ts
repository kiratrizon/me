...
class MeController extends Controller {
  ...
  public sendMail: HttpDispatch = async ({ request }) => {
    ...
    try {
      const cacheLimit = ((await Cache.get(`resend_limit`)) || {
        date: now,
        count: 0,
  ...
  public vc: HttpDispatch = async ({ request }) => {
    ...
    const tokenWithUid =
      (await Cache.get(`agora_token_${sessId}`)) ||
    ...
    const now = date("Y-m-d");
    const cacheLimit = ((await Cache.get(`vc_limit_${sessId}`)) || {
      date: now,
      count: 0,
    }) as { date: string; count: number };
    if (cacheLimit) {
    ...
    if (cacheLimit.count < 10) {
      try {
        if (
          (config("app").env !== "local" && cacheLimit.count < 10) ||
          (config("app").env === "local" && cacheLimit.count < 3)
        ) {
          await MeController.resendClient.emails.send({
